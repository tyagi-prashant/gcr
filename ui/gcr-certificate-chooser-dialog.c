...
{       GcrCertificateChooserDialog *self = GCR_CERTIFICATE_CHOOSER_DIALOG (data); 
       ...
       // gtk_file_chooser_set_preview_widget_active(self->page1_file_chooser, FALSE);
		...
		gtk_label_set_text(self->cert_label, gcr_parser_get_parsed_label(parser));
	    ...
	    self->key_uri = filename;
            gtk_label_set_text(self->key_label, "Key selected");
                         ...
                         gpointer *data_main)
{
	...
	GcrCertificateChooserDialog *self = GCR_CERTIFICATE_CHOOSER_DIALOG (data_main);
        ...
        gtk_file_chooser_set_preview_widget_active(self->page1_file_chooser, TRUE);
	...
	while (gcr_viewer_count_renderers(self->viewer))
        ...
        if(unlock != NULL) {
               g_object_set(unlock, "label", "Please Unlock");
...
on_page1_update_preview(GtkWidget *widget, gpointer *data_main)
{
	...
	GtkFileChooser *chooser = GTK_FILE_CHOOSER(widget);
        ...
        gsize n_data;
        GBytes *bytes;
        gtk_label_set_text(self->cert_label, "No certificate selected yet");
        gtk_label_set_text(self->key_label, "No key selected yet");
...
gcr_certificate_chooser_dialog_constructed (GObject *obj)
{
	...
	self->hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 2);
        gtk_container_add(GTK_CONTAINER(content), self->hbox);
	self->vbox1 = gtk_box_new(GTK_ORIENTATION_VERTICAL, 2);
	gtk_container_add(GTK_CONTAINER(self->hbox), self->vbox1);
        self->viewer = gcr_viewer_new();

	self->cert_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 2);
	gtk_container_add(GTK_CONTAINER(self->vbox1), self->cert_hbox);
	self->cert_image = gtk_image_new_from_gicon(g_themed_icon_new(GCR_ICON_CERTIFICATE), GTK_ICON_SIZE_DIALOG);
	gtk_container_add(GTK_CONTAINER(self->cert_hbox), self->cert_image);
	self->cert_label = gtk_label_new("No certificate selected yet");
	gtk_container_add(GTK_CONTAINER(self->cert_hbox), self->cert_label);

	self->key_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 2);
	gtk_container_add(GTK_CONTAINER(self->vbox1), self->key_hbox);
	self->key_image = gtk_image_new_from_gicon(g_themed_icon_new(GCR_ICON_KEY), GTK_ICON_SIZE_DIALOG);
	gtk_container_add(GTK_CONTAINER(self->key_hbox), self->key_image);
	self->key_label = gtk_label_new("No key selected yet");
	gtk_container_add(GTK_CONTAINER(self->key_hbox), self->key_label);

        self->notebook = gtk_stack_new();
        gtk_stack_set_transition_type(self->notebook,GTK_STACK_TRANSITION_TYPE_NONE);
        gtk_container_add(GTK_CONTAINER(self->hbox), self->notebook);
...
*/
       ...
       // g_signal_handler_disconnect (self->parser, sig);
